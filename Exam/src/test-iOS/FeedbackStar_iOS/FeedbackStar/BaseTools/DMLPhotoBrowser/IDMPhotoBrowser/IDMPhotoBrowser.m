...
#define SPH(value) ((((DeviceWidth - 320) / 320.0) * ((MULTI - 1) / 0.29) + 1) * (value))


//PHPhotoManager
static PHImageManager * _photoManager = nil;
@implementation PHPhotoManager

+(PHImageManager *)shareManager
{
    static dispatch_once_t oncelocation;
    dispatch_once(&oncelocation, ^{
        _photoManager = [[PHImageManager alloc] init];
    });
...
@interface IDMPhotoBrowser () {
    ...
    // AssetDictionary
    NSMutableDictionary *_photoAssetDictionary;
...
- (id)init {
    if ((self = [super init])) {
        ...
        _photoAssets = [NSMutableArray new];
        _photoAssetDictionary = [NSMutableDictionary new];
...
- (id)initWithAssets:(NSArray *)photoAssetArray animatedFromView:(UIView*)view {
    if ((self = [self init])) {
        _photoAssets = [[NSMutableArray alloc] initWithArray:photoAssetArray];
        _senderViewForAnimation = view;
    }
    return self;
}
...
- (void)releaseAllUnderlyingPhotos {
    ...
    for (NSNumber *key in _photoAssetDictionary.allKeys) {
        IDMPhoto *p = [_photoAssetDictionary objectForKey:key];
...
- (void)performPresentAnimation {
    ...
    void (^completion)() = ^() {
        ...
        [self getPhotoWithAssetIndex:_currentPageIndex isHeightQuality:YES];
    };
...
- (void)viewDidLoad {
    ...
    UIImage *actionButtonImage = (_actionImage == nil) ?
    [UIImage imageNamed:@"IDMPhotoBrowser.bundle/images/IDMPhotoBrowser_action.png"]          : _actionImage;
    ...
    UIImage *actionButtonSelectedImage = (_actionselectedImage == nil) ?
    [UIImage imageNamed:@"IDMPhotoBrowser.bundle/images/IDMPhotoBrowser_actionSelected.png"]          : _actionselectedImage;
    ...
    _counterLabel.font = [UIFont systemFontOfSize:SPH(14)];
    ...
    _actionButton = [[UIBarButtonItem alloc] initWithCustomView:[self customToolbarButtonImage:actionButtonImage
                                                                                 imageSelected:actionButtonSelectedImage
                                                                                        action:@selector(photoSelected:)]];
    // Gesture
...
- (id<IDMPhoto>)photoAtIndex:(NSUInteger)index {
    if (_photoAssets.count) {
        return [self getPhotoWithAssetIndex:index isHeightQuality:NO];
    }
    return _photos[index];
}

- (IDMPhoto *)getPhotoWithAssetIndex:(NSInteger)index isHeightQuality:(BOOL) isHeight {
    __block IDMPhoto *photo = [_photoAssetDictionary objectForKey:@(index)];
    if (!photo) {
        if (index < _photoAssets.count) {
            PHAsset *asset = _photoAssets[index];
            ...
            [[PHPhotoManager shareManager] requestImageForAsset:asset targetSize:size contentMode:PHImageContentModeAspectFill options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                photo = [[IDMPhoto alloc]initWithImage: result];
                if ([self.delegate respondsToSelector:@selector(photoBrowser:isSelectedAtIndex:)]) {
                    photo.selected = [self.delegate photoBrowser:self isSelectedAtIndex:index];
                }
            }];
            [_photoAssetDictionary setObject:photo forKey:@(index)];
        }
    }else {
        if (isHeight) {
            PHAsset *asset = _photoAssets[index];
            ...
            [[PHPhotoManager shareManager] requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFill options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                IDMPhoto *photo = [self getPhotoWithAssetIndex:_currentPageIndex isHeightQuality:NO];
//                [photo resetImage:result];
                IDMZoomingScrollView *page = [self pageDisplayingPhoto: photo];
                page.contentMode = UIViewContentModeScaleAspectFill;
                [page.photoImageView setImage:result];
            }];
        }
    }
    return photo;
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    UIButton *actionBtn = _actionButton.customView;
    actionBtn.selected = ((IDMPhoto*)currentPhoto).selected;
...
- (void)photoSelected:(UIButton *)btn {
    BOOL isCanSelect = NO;
    if ([_delegate respondsToSelector:@selector(photoBrowser:isCanSelectAtIndex:)]) {
        isCanSelect = [_delegate photoBrowser:self isCanSelectAtIndex:_currentPageIndex];
    }
    ...
    photo.selected = btn.selected;
    
    if ([_delegate respondsToSelector:@selector(photoBrowser:index:didSelectPhoto:)]) {
        [_delegate photoBrowser:self index:_currentPageIndex didSelectPhoto:isSelected];
    }
}
