...
+ (void)showText:(NSString *)text duration:(NSTimeInterval)timeInterval {
    if ([text isEqual:@""] || text == nil || text == NULL) {
        return;
    }
    [[self sharedView] showImage:nil status:text duration:timeInterval];
}
...
- (void)updatePosition {
    ...
    if(string) {
        CGRect stringRect = [string boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 100, 300) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObjectsAndKeys:self.stringLabel.font,NSFontAttributeName, nil] context:nil];
        stringWidth = stringRect.size.width;
        stringHeight = stringRect.size.height;
        if (imageUsed)
            hudHeight = 80 + stringHeight;
        else
            hudHeight = ([UIScreen mainScreen].bounds.size.width / 320.0) * (10) + stringHeight;
        
        if(stringWidth > hudWidth)
            hudWidth = ceil(stringWidth/2)*2;
        
        CGFloat labelRectY = imageUsed ? 66 : ([UIScreen mainScreen].bounds.size.width / 320.0) * (5);
...
- (UIFont *)hudFont {
    ...
    return [UIFont systemFontOfSize:([UIScreen mainScreen].bounds.size.width / 320.0) * (14)];
}
