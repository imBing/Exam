...
@synthesize isStarting;

- (void)startAnimating
{
    isStarting = YES;
    animationType = MBProgressHUDAnimationZoom; //MBProgressHUDAnimationZoom;
    useAnimation = YES;
...
- (void)stopAnimatingWithAnimated:(BOOL)bAnimated
{
    isStarting = NO;
    useAnimation = bAnimated;
    [self hideUsingAnimation:useAnimation];
}

- (void)tipDismiss:(NSTimer *)timer
{
    [timer invalidate];
    [self hideUsingAnimation:YES];
}

-(void)showTips:(float)duration finishedCallback:(id)finishedCallback
{
...
- (void)showTips:(float)duration
{
    [self showTips:duration finishedCallback:nil];
}
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        _delegateEx = nil;
        //NSLog(@"frame  111111111111  %@",NSStringFromCGRect(frame));
        // Set default values for properties
        self.animationType = MBProgressHUDAnimationZoom;//;MBProgressHUDAnimationZoom;//MBProgressHUDAnimationFade;
        ...
        //		NSLog(@"detailsLabel frame%@",NSStringFromCGRect([detailsLabel frame]) );
        taskInProgress = NO;
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        CGSize dims = [self.labelText adaptSizeWithFont:self.labelFont constrainedToSize:CGSizeMake(999, frame.size.height) lineBreakMode:NSLineBreakByWordWrapping];
        ...
        if (nil != self.detailsLabelText) {
            // Get size of label text
            CGSize dims = [self.detailsLabelText adaptSizeWithFont:self.detailsLabelFont constrainedToSize:CGSizeMake(999, frame.size.height) lineBreakMode:NSLineBreakByWordWrapping];
...
- (void)resetOffset:(CGPoint)offset {
...
//    if(_delegateEx && [_delegateEx respondsToSelector:@selector(hudWasHiddenEx:)]) {
//        SuppressPerformSelectorLeakWarning([_delegateEx performSelector:@selector(hudWasHiddenEx:) withObject:self afterDelay:0.2]);
//    }
//    
//    if(_finishedCallback && [_finishedCallback respondsToSelector:@selector(hudFinishedCallback:)]) {
//        SuppressPerformSelectorLeakWarning( [_finishedCallback performSelector:@selector(hudFinishedCallback:) withObject:self afterDelay:0.2]);
//       
//    }
...
- (void)drawRect:(CGRect)rect {
    ...
    //	BMLog(@"******************allRect = %@", NSStringFromCGRect(allRect));
...
- (void)fillRoundedRect:(CGRect)rect inContext:(CGContextRef)context {
    ...
    //    CGFloat gray[4] = {0.75f, 0.05f, 0.05f, 1.0f};
...
- (void)deviceOrientationDidChange:(NSNotification *)notification {
    ...
    //	DBMLog(@"deviceOrientationDidChange");
